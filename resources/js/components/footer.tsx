...
function normalizeUrl(v?: string | null): string | null {
    ...
    try {
        return new URL('https://' + v.replace(/^\/+/, '')).href;
    } catch {
        return null;
    }
}
...
export default function FooterCard(props: Props) {
    ...
    const addressLinesRaw: string[] = props.addressLines ?? portal?.addressLines ?? ['ul. Chorzowska 44c', '44-100 Gliwice'];

    const phone: string | undefined = props.phone ?? portal?.phone ?? '32 440 15 54';

    const email: string | undefined = props.email ?? portal?.email ?? 'kontakt@seniornaplus.pl';

    const privacyPolicyUrl: string | null = props.privacyPolicyUrl ?? portal?.privacyPolicyUrl ?? null;

    const termsOfServiceUrl: string | null = props.termsOfServiceUrl ?? portal?.termsOfServiceUrl ?? null;
    ...
    const addressLines: string[] = React.useMemo(() => {
        ...
        const first = String(addressLinesRaw[0]).replace(/(\b\d+\s*[A-Za-z]?(?:\/\d+\s*[A-Za-z]?)?)\s+/, '$1\n');
    ...
    return (
        <footer
            ref={footerRef}
            role="contentinfo"
            className="relative right-1/2 left-1/2 -mr-[50vw] -ml-[50vw] w-screen border-t border-black/10 bg-blush"
        >
            <div className="p-6 text-center md:p-8">
                ...
                <div className="mt-3 space-y-1 text-[17px]">
                    ...
                    {phone && (
                        <div>
                            tel.&nbsp;
                            <a href={`tel:${String(phone).replace(/\s+/g, '')}`} className="underline underline-offset-2 hover:opacity-80">
                                {phone}
                            </a>
                        </div>
                    )}
                    ...
                    {email && (
                        <div>
                            <a href={`mailto:${email}`} className="underline underline-offset-2 hover:opacity-80">
                                {email}
                            </a>
                        </div>
                    )}
                ...
                <div className="mt-4 flex flex-wrap justify-center gap-4 text-sm text-gray-600">
                    {privacyPolicyUrl && (
                        <a href={privacyPolicyUrl} className="underline underline-offset-2 transition-opacity hover:opacity-80">
                            Polityka Prywatności
                        </a>
                    )}
                    {termsOfServiceUrl && (
                        <a href={termsOfServiceUrl} className="underline underline-offset-2 transition-opacity hover:opacity-80">
                            Warunki Korzystania
                        </a>
                    )}
                </div>
...
function SocialIcon({ href, label, children }: React.PropsWithChildren<{ href: string; label: string }>) {
    return (
        <a
            href={href}
            aria-label={label}
            ...
            className="inline-flex h-10 w-10 items-center justify-center rounded-md bg-coral text-white shadow-sm ring-1 ring-black/10 transition hover:opacity-95 focus-visible:ring-2 focus-visible:ring-coral focus-visible:outline-none"
        >
            {children}
        </a>
    );
