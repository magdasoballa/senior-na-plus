...
export default function Form() {
    const { form: current } = usePage<{ form: FormRow | null }>().props;
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        if (isEdit) form.put(`${BASE}/${current!.id}`, { preserveScroll: true });
        else form.post(`${BASE}`, { preserveScroll: true });
    };
    const submitAndContinue = () => {
        form.transform((d) => ({ ...d, redirectTo: 'continue' as const }));
        if (isEdit) {
            form.put(`${BASE}/${current!.id}`, { preserveScroll: true, onFinish: () => form.transform((d) => d) });
        } else {
