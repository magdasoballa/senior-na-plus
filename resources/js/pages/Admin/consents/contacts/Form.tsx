...
export default function Form() {
    const { contact, flash } = usePage<{ contact: Contact | null; flash?: { success?: string; error?: string } }>().props
    ...
    // lokalny komunikat po „Kontynuuj”
    const [saved, setSaved] = useState(false)

    const submit = (e: React.FormEvent) => {
        e.preventDefault()
        if (isEdit) {
            form.put(`${BASE}/${contact!.id}`, { preserveScroll: true })
        } else {
    ...
    const submitAndContinue = () => {
        ...
        const opts = {
            preserveScroll: true,
            onSuccess: () => {
                // pokaż lokalny badge jeśli zostajemy na edycji
                setSaved(true)
        ...
        if (isEdit) form.put(`${BASE}/${contact!.id}`, opts)
    ...
    return (
        <AdminLayout>
            <main className="p-6">
                ...
                {/* lokalny „Zapisano zmiany” po Kontynuuj (gdy nie ma flasha) */}
                {saved && (
              ...
              />
                    ...
                    <div className="mt-8 flex items-center justify-end gap-3">
                        ...
                        <button
                            ...
                            className="rounded-lg bg-mint cursor-pointer px-4 py-2 font-semibold"
