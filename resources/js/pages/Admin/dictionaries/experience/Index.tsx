...
export default function Index(){
    const { experiences, filters } = usePage<{ experiences:Paginated<Row>; filters:{q?:string} }>().props
    ...
    const handleDelete = (id:number)=>{
        if(!confirm('Na pewno usunąć to doświadczenie?')) return
        setDeletingId(id)
        router.delete(`${BASE}/${id}`, {
            data: q ? { q } : {},
            preserveScroll: true,
            onSuccess: () => { if (q) router.get(BASE, { q }, { preserveState:true, replace:true }) },
    ...
    return (
        <AdminLayout>
            <main className="p-6">
                <div className="text-sm text-slate-500">Zasoby › Doświadczenia w opiece</div>
                <div className="mt-1 flex items-center justify-between">
                    <p className="text-2xl font-bold">Doświadczenia w opiece</p>
                    <Link href={`${BASE}/create`} className="rounded-lg bg-mint px-3 py-1 font-semibold text-white hover:brightness-110">
                        Utwórz Doświadczenie w opiece
                    </Link>
                </div>
                ...
                <div className="mt-4 overflow-hidden rounded-xl border bg-white">
                        ...
                        {experiences.data.map(r=>(
                            <tr key={r.id} className="border-t">
                                ...
                                <td className="px-4 py-3">
                                    ...
                                    {r.is_visible_pl ?  <CheckCircle2 className="h-5 w-5 text-emerald-600" aria-hidden />
                                        :  <XCircle className="h-5 w-5 text-rose-600" aria-hidden />}</div>
                                </td>
                                <td className="px-4 py-3">
                                    <div className="flex justify-center">

                                    {r.is_visible_de ?  <CheckCircle2 className="h-5 w-5 text-emerald-600" aria-hidden />
                                        :  <XCircle className="h-5 w-5 text-rose-600" aria-hidden />}</div>
                                </td>
                                <td className="px-4 py-3">
                    ...
                    <div className="flex items-center justify-between border-t px-4 py-3 text-sm text-slate-600">
                        <span>{rangeInfo(experiences)}</span>
                        <nav className="flex items-center gap-1">
                            {experiences.links.map((l,i)=>(
