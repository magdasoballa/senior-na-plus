            ...
            <div className="max-w-5xl space-y-6 p-6">
                ...
                <section className="space-y-4 rounded-lg bg-[#F5F5F4] p-6">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label className="text-sm font-medium text-foreground/80">Imię i nazwisko</label>
                        ...
                        {application.type === 'application' && application.language_level && (
                            <div>
                                <label className="text-sm font-medium text-foreground/80">Poziom języka</label>
                ...
                {/* Dodatkowe informacje (tylko dla zwykłych aplikacji) */}
                {application.type === 'application' && (
                    <section className="space-y-4 rounded-lg bg-[#F5F5F4] p-6">
                        <h2 className="text-xl font-semibold text-coral">Dodatkowe informacje</h2>
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                <label className="text-sm font-medium text-foreground/80">Dodatkowy język</label>
                                <p className="text-foreground">{application.additional_language || '—'}</p>
                            </div>
                            <div>
                                <label className="text-sm font-medium text-foreground/80">Zawód wyuczony</label>
                                <p className="text-foreground">{application.learned_profession || '—'}</p>
                            </div>
                            <div>
                                <label className="text-sm font-medium text-foreground/80">Zawód wykonywany</label>
                ...
                {/* Doświadczenie zawodowe (tylko dla zwykłych aplikacji) */}
                {application.type === 'application' && (
                    <section className="space-y-4 rounded-lg bg-[#F5F5F4] p-6">
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            {application.experience && (
                                <div>
                                    <label className="text-sm font-medium text-foreground/80">Doświadczenie jako opiekun</label>
                                    <p className="text-foreground">{application.experience}</p>
                                </div>
                            )}
                            {application.salary_expectations && (
                                <div>
                                    <label className="text-sm font-medium text-foreground/80">Oczekiwania finansowe</label>
                                    <p className="text-foreground">{`${application.salary_expectations} €`}</p>
                                </div>
                            )}
                            <div className="col-span-2">
                                <label className="text-sm font-medium text-foreground/80">Dodatkowe umiejętności</label>
                                <ul className="list-disc pl-5 text-foreground">
                                    {application.first_aid_course !== undefined && (
                                        <li>Kurs pierwszej pomocy: {application.first_aid_course ? 'Tak' : 'Nie'}</li>
                                    )}
                                    {application.medical_caregiver_course !== undefined && (
                                        <li>Kurs opiekuna medycznego: {application.medical_caregiver_course ? 'Tak' : 'Nie'}</li>
                                    )}
                                    {application.care_experience !== undefined && (
                                        <li>Doświadczenie w opiece: {application.care_experience ? 'Tak' : 'Nie'}</li>
                                    )}
                                    {application.housekeeping_experience !== undefined && (
                                        <li>Doświadczenie w porządkach domowych: {application.housekeeping_experience ? 'Tak' : 'Nie'}</li>
                                    )}
                                    {application.cooking_experience !== undefined && (
                                        <li>Doświadczenie w gotowaniu: {application.cooking_experience ? 'Tak' : 'Nie'}</li>
                                    )}
                                    {application.driving_license !== undefined && (
                                        <li>Prawo jazdy: {application.driving_license ? 'Tak' : 'Nie'}</li>
                                    )}
                                    {application.smoker !== undefined && (
                            ...
                            {application.references_path && (
                                <div>
                                    <label className="text-sm font-medium text-foreground/80">Referencje</label>
                ...
                <section className="space-y-4 rounded-lg bg-[#F5F5F4] p-6">
                    ...
                    <div className="space-y-2">
                        {application.consent1 !== undefined && <p>Zgoda 1: {application.consent1 ? 'Zaakceptowana' : 'Niezaakceptowana'}</p>}
                        {application.consent2 !== undefined && <p>Zgoda 2: {application.consent2 ? 'Zaakceptowana' : 'Niezaakceptowana'}</p>}
                        {application.consent3 !== undefined && <p>Zgoda 3: {application.consent3 ? 'Zaakceptowana' : 'Niezaakceptowana'}</p>}
                    </div>
                </section>

                {/* Dodatkowe metadane (tylko dla szybkich aplikacji) */}
                {application.type === 'quick_application' && (
                    <section className="space-y-4 rounded-lg bg-[#F5F5F4] p-6">
                        <h2 className="text-xl font-semibold text-coral">Metadane</h2>
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            {application.url && (
                                <div>
                                    <label className="text-sm font-medium text-foreground/80">URL</label>
                                    <p className="text-foreground">{application.url}</p>
                                </div>
                            )}
                            {application.ip && (
                                <div>
                                    <label className="text-sm font-medium text-foreground/80">IP</label>
                                    <p className="text-foreground">{application.ip}</p>
                                </div>
                            )}
                            {application.user_agent && (
                                <div>
                                    <label className="text-sm font-medium text-foreground/80">User Agent</label>
                                    <p className="text-foreground">{application.user_agent}</p>
                                </div>
                            )}
                        </div>
                    </section>
