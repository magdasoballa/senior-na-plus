...
import { CheckCircle2, Pencil, XCircle, Trash2, Eye } from 'lucide-react'
...
export default function Index(){
    ...
    const del = (id:number)=>{
        if(!confirm('Na pewno usunąć wymaganie?')) return
        setDeleting(id)
        router.delete(`${BASE}/${id}`, { preserveScroll:true, onFinish:()=>setDeleting(null) })
    }

    return (
        <AdminLayout>
            <main className="p-6">
                <div className="text-sm text-slate-500">Oferty › Wymagania</div>
                <div className="mt-1 flex items-center justify-between">
                    <p className="text-2xl font-bold">Wymagania</p>
                    <Link href={`${BASE}/create`} className="rounded-lg bg-mint px-3 py-1 font-semibold text-white hover:brightness-110">Utwórz Wymaganie</Link>
                </div>
                ...
                <div className="mt-4 overflow-hidden rounded-xl border bg-white">
                    <table className="w-full text-sm">
                            ...
                            <tr key={r.id} className="border-t">
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex justify-end gap-2">
                                        ...
                                        <button onClick={()=>del(r.id)} disabled={deleting===r.id} className="rounded border px-2 py-1 text-rose-600 hover:bg-rose-50" title="Usuń">
                        ...
                        {requirements.data.length===0 && (
                            <tr><td className="px-4 py-8 text-center text-slate-500" colSpan={4}>Brak rekordów</td></tr>
                        )}
                        </tbody>
                    </table>
...
function rangeInfo<T>(p:Paginated<T>){
